...
// Include our new improved components
...
// Function to apply random horizontal flips for data augmentation
void randomHorizontalFlip(Eigen::MatrixXf& image, float probability = 0.5f) {
    std::random_device rd;
    std::mt19937 gen(rd());
    std::uniform_real_distribution<float> dis(0.0f, 1.0f);
    
    if (dis(gen) < probability) {
        // Image is in column vector format [3*32*32, 1]
        // Need to flip each channel separately
        const int height = 32;
        const int width = 32;
        ...
        for (int c = 0; c < channels; c++) {
            ...
            for (int h = 0; h < height; h++) {
                ...
                for (int w = 0; w < width / 2; w++) {
                    std::swap(image(rowOffset + w, 0), image(rowOffset + width - 1 - w, 0));
                }
            }
        }
    }
...
Eigen::MatrixXf createMiniBatch(const std::vector<Eigen::MatrixXf>& images, 
                               ...
                               int startIdx, int batchSize, bool augment = false) {
    ...
    for (int i = 0; i < actualBatchSize; i++) {
        ...
        Eigen::MatrixXf image = images[indices[startIdx + i]];
...
int main() {
    std::cout << "Creating CNN CIFAR-10 classifier with improved components..." << std::endl;
    ...
    model.addLayer(std::make_shared<ImprovedConvLayer>("conv1", inputChannels, inputHeight, inputWidth, 32, 3, 1, 1));
    model.addLayer(std::make_shared<ChannelBatchNormLayer>("bn1", 32, inputHeight, inputWidth));
    model.addLayer(std::make_shared<ReLULayer>("relu1", 32 * inputHeight * inputWidth, 32 * inputHeight * inputWidth));
    model.addLayer(std::make_shared<MaxPoolingLayer>("pool1", 32, inputHeight, inputWidth, 2, 2));
    
    // Output dimensions after first block
    const int h1 = (inputHeight - 2) / 2 + 1; // 16
    const int w1 = (inputWidth - 2) / 2 + 1;  // 16
    
    // Second convolutional block
    model.addLayer(std::make_shared<ImprovedConvLayer>("conv2", 32, h1, w1, 64, 3, 1, 1));
    model.addLayer(std::make_shared<ChannelBatchNormLayer>("bn2", 64, h1, w1));
    model.addLayer(std::make_shared<ReLULayer>("relu2", 64 * h1 * w1, 64 * h1 * w1));
    model.addLayer(std::make_shared<MaxPoolingLayer>("pool2", 64, h1, w1, 2, 2));
    
    // Output dimensions after second block
    const int h2 = (h1 - 2) / 2 + 1; // 8
    const int w2 = (w1 - 2) / 2 + 1; // 8
    
    // Third convolutional block
    model.addLayer(std::make_shared<ImprovedConvLayer>("conv3", 64, h2, w2, 128, 3, 1, 1));
    model.addLayer(std::make_shared<ChannelBatchNormLayer>("bn3", 128, h2, w2));
    model.addLayer(std::make_shared<ReLULayer>("relu3", 128 * h2 * w2, 128 * h2 * w2));
    model.addLayer(std::make_shared<MaxPoolingLayer>("pool3", 128, h2, w2, 2, 2));
    
    // Output dimensions after third block
    const int h3 = (h2 - 2) / 2 + 1; // 4
    const int w3 = (w2 - 2) / 2 + 1; // 4
    const int flattenSize = 128 * h3 * w3; // 2048
    
    // Fully connected layers
    model.addLayer(std::make_shared<DenseLayer>("fc1", flattenSize, 512));
    model.addLayer(std::make_shared<ReLULayer>("relu4", 512, 512));
    model.addLayer(std::make_shared<DropoutLayer>("dropout1", 512, 0.5f));
    ...
    std::cout << "Starting training with CNN architecture..." << std::endl;
    std::cout << "Model architecture: " << std::endl;
    
    // Print model architecture
    for (const auto& layer : model.getLayers()) {
        std::cout << "  - " << layer->getName() << " (" << layer->getType() << "): " 
                  << layer->getInputSize() << " -> " << layer->getOutputSize() << std::endl;
    }
    ...
    for (int epoch = 0; epoch < epochs; ++epoch) {
        ...
        for (size_t b = 0; b < trainImages.size(); b += batchSize) {
            // Create mini-batch with data augmentation
            Eigen::MatrixXf batchInput = createMiniBatch(trainImages, indices, b, batchSize, true);
        ...
        if (validationAccuracy > bestValidationAccuracy) {
            ...
            patienceCounter = 0;
            std::string checkpointPath = "I:/ZyraAI/checkpoints/cifar10_cnn_best.bin";
            ...
            if (epoch % 10 == 9) {
                std::string checkpointPath = "I:/ZyraAI/checkpoints/cifar10_cnn_epoch_" + std::to_string(epoch + 1) + ".bin";
                saveModelCheckpoint(model, checkpointPath);
            }
        }
