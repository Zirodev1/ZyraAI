...
int main() {
  std::cout << "Creating MNIST ResNet Classifier (with Residual Connections)..."
            << std::endl;

  // Create checkpoint directory
  std::string checkpointDir = "checkpoints_residual_network";
  createDirectory(checkpointDir);

  // Create the model
  ZyraAIModel model;

  // Define ResNet architecture with residual connections:
  // 1x28x28 (input) -> 16x28x28 (Conv) -> 16x14x14 (Pool) ->
  // ResBlock(16->32) -> ResBlock(32->64) -> SimpleResBlock(64->64) ->
  ...
  // Using SimpleResidualBlock for variation and comparison
  model.addLayer(std::make_shared<SimpleResidualBlock>(
      "res_block3", 64, 7, 7, 3, 1)); // 64 channels, same dimensions
